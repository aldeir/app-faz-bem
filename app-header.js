// /js/app-header.js (Versão 8.0 - Definitiva, com segurança e UX melhorada)
...
export async function injectHeader() {
    ...
    const verificationBanner = (userSession && !userSession.isVerified) 
        ? `<div class="bg-yellow-300 text-yellow-800 text-center text-sm p-2">
               A sua conta está pendente de verificação. Por favor, verifique o seu e-mail.
               <a href="verificar-email.html" class="font-bold underline hover:text-yellow-900">Precisa de ajuda?</a>
           </div>`
        : '';

    headerContainer.innerHTML = `
        <style>
            .menu-item { display: flex; align-items: center; text-align: left; padding: 0.5rem 1rem; font-medium; margin: 0.25rem; border-radius: 0.375rem; color: #374151; transition: background-color 0.2s, color 0.2s; }
    ...
    if (userSession?.auth) {
        ...
        if (!isVerified) {
            if (!isPublicPage) {
                // Redireciona para a página de verificação se tentar aceder a uma página protegida
                window.location.href = 'verificar-email.html';
                return false; 
            }
            // Em páginas públicas, mostra apenas o botão de sair
